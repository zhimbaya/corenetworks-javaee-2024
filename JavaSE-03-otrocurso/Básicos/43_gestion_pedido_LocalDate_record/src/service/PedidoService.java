...
public class PedidoService {
	...
	public Cesta recientes () {
		...
		LocalDate fechaReciente=cesta.fechaPedido();
		for(Cesta c:cestaList) {
			if(c.fechaPedido().isAfter(fechaReciente)) { //si encontramos una fecha más reciente, actualizamos variables
				fechaReciente=c.fechaPedido();
				cesta=c;
			}
		}
		return cesta;
	...
	public ArrayList<Cesta> anteriores (int meses) {
		...
		for(Cesta c:cestaList) {
			if(c.fechaPedido().isBefore(referencia)) {
				res.add(c);
			}
		}
		return res;
